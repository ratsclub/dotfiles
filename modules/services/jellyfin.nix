...
in
{
  ...
  config = mkIf cfg.enable {
    systemd.services.jellyfin = {
      ...
      serviceConfig = rec {
        ...
        ExecStart = "${cfg.package}/bin/jellyfin --configdir '/etc/jellyfin' --logdir '/var/log/${LogsDirectory}' --datadir '/var/lib/${StateDirectory}' --cachedir '/var/cache/${CacheDirectory}'";
        Restart = "on-failure";
        TimeoutSec = 15;
  ...
  meta.maintainers = with lib.maintainers; [ ratsclub ];
}
